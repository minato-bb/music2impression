import argparse
import inner_prodact
import inner_prodact_copy
import norm
#内積計算
# python main.py music_impression -i 曲名


def music_impression(name):
    word = ["cool", "relax", "excited", "sad", "fierce"]
    if name not in word:
        inner_prodact.inner_product(name)
    else:
        inner_prodact_copy.inner_product(name)
